...
| `Reply` | `source`, `body`, `tags` β€” note: **no** `to`/`canReply`/`canReact`, matching the invariant that a Reply's own addressing fields always stay blank (see [Reply](/docs/activities/reply/)) |
...
- **Password change** is a separate, owner-only path: `object.password = { current, new }`, requires `bcrypt.compare` against the stored hash, `new` must be β‰₯8 chars. Admins cannot change another user's password through `Update`.
